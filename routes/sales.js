...
//Direct User to Edit Product Form/pug
router.get('/sale-edit/:id',(req, res, next)=> {
    let prodId = req.params.id | 0; //required info, parameters, .id is this product's id, |0 converts to a number
    let product = myProducts.find(product=>product.id == prodId);
    res.render('sale-edit',{ product: product});
});

//Direct User to Delete Product Form
router.get('/sale-delete/:id',(req, res, next)=> {
    ...
    //myProducts = myProducts.filter(product=>product.id !== prodId);
    res.redirect('/sales', { product: deletedprod[0]});
});
...
myProducts.push(
    ...
    new Product(2,'Shirt',75.99,'34"','/images/shirt.jpg'),
